...
function showSplashAndMainWindow() {
  ...
  const startingMessage_t = "Starting Terminal";
  const searchMessage_t = "Searching for Index";
  const welcomeMessage_t = "Index Found";
  ...
  const splashHTML = `
    ...
    <body>
      ...
      <script>
        ...
        const searchMessage = ${JSON.stringify(searchMessage_t)};
        const interval = setInterval(() => {
          count--;
          if      (count === 3)  { text.textContent = "Loading Resources"; }
          else if (count === 2)  { text.textContent = "Initializing Modules"; }
          else if (count === 1)  { text.textContent = startingMessage + " ..."; }
          else if (count === 0)  { text.textContent = searchMessage + " ..."; }
          else if (count === -1) {
            text.textContent = "";
