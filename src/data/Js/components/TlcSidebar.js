...
class TlcSidebar extends HTMLElement {
  ...
  connectedCallback() {
    ...
    window.addEventListener('tlc:langchange', () => {
      ...
      const searchEl = sr?.getElementById('search-input');
      if (searchEl) searchEl.placeholder = t('sidebar.search');
      const barTitle = sr?.getElementById('bar-title');
      if (barTitle) barTitle.textContent = this._page === 'zones' ? t('sidebar.zones') : t('sidebar.locations');
    });
  ...
  _buildZones() {
    ...
    sr.getElementById('bar-title').textContent   = t('sidebar.zones');
  ...
  _showLocations(zone) {
    ...
    sr.getElementById('bar-title').textContent = t('sidebar.locations');
  ...
  _setBoat() {
    ...
    if (isNaN(gx) || isNaN(gy)) {
      window.tlcNotify(t('sidebar.invalid_coords'), 'warn');
      return;
    }
