...
const HC_MEMORIES = [
  ...
  { id:'music_notes',        name:'Music Notes',           adaptability:0, creativity:6, communication:4, discipline:0, empathy:0, focus:0, leadership:0, logic:0, patience:0, wisdom:0 },
...
const HC_BASE = { height: 0, weight: 0, life_expectancy: 0 };
...
class TlcHumanCalc extends HTMLElement {
  ...
  connectedCallback() {
    if (this._initialized) return;   // prevent double-init on DOM re-attach
  ...
  _loadInv() {
    try {
      const rawStock = window.TLC_STATE?.get('humanCalcStock') || {};
      const rawInv   = window.TLC_STATE?.get('humanCalcInv')   || {};
      this._stock = JSON.parse(JSON.stringify(rawStock));
      this._inv   = JSON.parse(JSON.stringify(rawInv));
      this._humanStatus = (window.TLC_STATE?.get('humanCalcStatus')) || {};
    } catch(e) { this._inv = {}; this._humanStatus = {}; }
  }
  _saveInv() {
    try {
      const invCopy   = JSON.parse(JSON.stringify(this._inv));
      const stockCopy = JSON.parse(JSON.stringify(this._stock));
      window.TLC_STATE?.set('humanCalcInv',   invCopy);
      window.TLC_STATE?.set('humanCalcStock', stockCopy);
      window.TLC_STATE?.save().catch(e => console.warn('[HumanCalc] save error:', e));
    } catch(e) { console.error('[HumanCalc] _saveInv exception:', e); }
  }
  _saveStatus() {
    try {
      window.TLC_STATE?.set('humanCalcStatus', JSON.parse(JSON.stringify(this._humanStatus)));
      window.TLC_STATE?.save().catch(e => console.warn('[HumanCalc] save error:', e));
    } catch(e) { console.warn('[HumanCalc] _saveStatus error:', e); }
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="hc-header">
        ...
        <div class="hc-title">${t('hc.title')}</div>
        <button class="hc-close" id="hc-close">${this.hasAttribute('standalone') ? '‹' : '✕'}</button>
      </div>

      <!-- Tabs -->
      <div class="hc-tabs">
        <div class="hc-tab ${this._tab==='inventory'?'active':''}" data-tab="inventory">${'📦 '+t('hc.tab_inventory')}</div>
        <div class="hc-tab ${this._tab==='results'?'active':''}" data-tab="results">${'🧬 '+t('hc.tab_humans')}</div>
      </div>
...
`;  // no totals bar on inventory tab
  ...
  _renderResults(totals) {
    ...
    const cards = humans.map(h => {
      ...
      const baseReqs = [
        { label:t('hc.height'), need: HC_BASE.height + (h.height||0), have: totals.height },
        { label:t('hc.weight'), need: HC_BASE.weight + (h.weight||0), have: totals.weight },
        { label:t('hc.life_exp'),   need: HC_BASE.life_expectancy + (h.life_expectancy||0), have: totals.life_expectancy },
      ].filter(b => b.need > 0);
      ...
      return `<div class="human-card ${can?'growable':''}${statusClass}" data-human="${h.name}" title="Left-click: details | Right-click: set status">
        ...
        ${can ? `<div class="growable-label">${t('hc.growable')}</div>` : ''}
      </div>`;
    });
    ...
    return `
      <div class="results-toolbar">
        ...
        ${[0,1,2,3,4].map(tier => `<button class="tier-btn ${this._filterTier===tier?'active':''}" data-tier="${tier}">${tier===0?t('hc.filter_all'):(t('hc.tier')+' '+tier)}</button>`).join('')}
        <label class="growable-toggle">
          <input type="checkbox" id="hc-growable" ${this._showOnlyGrowable?'checked':''}> ${t('hc.filter_growable')}
        </label>
      </div>
      <div class="hc-body">
        <div style="padding:10px 14px 0; font-size:11px; color:#4a7a8a;">
          ${growable} / ${humans.length} ${t('hc.filter_growable')}
        </div>
  ...
  _bindEvents() {
    ...
    sr.getElementById('hc-close').addEventListener('click', () => {
      // Save before closing
      this._saveInv();
      this._saveStatus();
      this.remove();
      // If opened as standalone page, go back to main menu
    ...
    sr.getElementById('hc-clear').addEventListener('click', () => {
      if (!confirm(t('hc.clear_inventory'))) return;
      this._inv = {};
      this._saveInv();
      this._render();
    });
  ...
  _openCtx(e, humanName) {
    ...
    menu.innerHTML = `
      <div class="hc-ctx-item ${status==='owned'?'active':''}" data-action="owned">${t('hc.mark_owned')}</div>
      <div class="hc-ctx-item ${status==='grow'?'active':''}"  data-action="grow">${t('hc.mark_grow')}</div>
  ...
  _buildDetailHTML(h) {
    ...
    // ── Auto-allocate: optimale Kombination mit minimalem Item-Verbrauch ──
    // Iterative Verbesserung: Greedy + lokale Optimierung ohne rekursives Branching
    const _autoAlloc = (items, attrs) => {
      // Nur relevante Items mit Inventar
      const relevant = items.filter(item =>
        this._inv[item.id] > 0 &&
        attrs.some(a => (item[a]||0) > 0 && (h[a]||0) > 0)
      );
      const needs = {};
      attrs.forEach(a => { if ((h[a]||0) > 0) needs[a] = h[a]; });
      const needAttrs = Object.keys(needs);
      if (!needAttrs.length) return {};

      // Effektivitäts-Score: wie viel deckt 1 Einheit eines Items ab (normalisiert)
      const score = (item, rem) =>
        needAttrs.reduce((s, a) => s + Math.min(item[a]||0, rem[a]||0), 0);

      // ── Schritt 1: Greedy mit dynamischer Sortierung ─────────────────
      const greedySolve = (itemPool) => {
        const alloc = {};
        const rem   = { ...needs };
        while (needAttrs.some(a => rem[a] > 0)) {
          // Wähle Item mit bestem aktuellen Score
          const pool = itemPool.filter(item =>
            this._inv[item.id] > (alloc[item.id]||0) &&
            needAttrs.some(a => (item[a]||0) > 0 && rem[a] > 0)
          );
          if (!pool.length) break;
          pool.sort((a, b) => score(b, rem) - score(a, rem));
          const best = pool[0];
          // Wie viele brauchen wir mindestens?
          let cnt = 0;
          needAttrs.forEach(a => {
            if ((best[a]||0) > 0 && rem[a] > 0)
              cnt = Math.max(cnt, Math.ceil(rem[a] / (best[a]||1)));
          });
          cnt = Math.min(cnt, this._inv[best.id] - (alloc[best.id]||0));
          if (cnt <= 0) break;
          alloc[best.id] = (alloc[best.id]||0) + cnt;
          needAttrs.forEach(a => {
            rem[a] = Math.max(0, rem[a] - (best[a]||0) * cnt);
          });
        }
        return alloc;
      };

      let best = greedySolve(relevant);
      const totalOf = (a) => Object.values(a).reduce((s,v)=>s+v,0);

      // ── Schritt 2: Lokale Optimierung — entferne überflüssige Items ──
      // Nach dem Greedy: prüfe ob einzelne Items komplett weggelassen werden können
      let improved = true;
      while (improved) {
        improved = false;
        for (const item of relevant) {
          if (!(best[item.id] > 0)) continue;
          // Wie viel tragen alle anderen Items bei?
          const withoutItem = { ...best, [item.id]: 0 };
          const stillCovers = needAttrs.every(a => {
            const tot = relevant.reduce((s, it) => s + (it[a]||0) * (withoutItem[it.id]||0), 0);
            return tot >= (needs[a]||0);
          });
          if (stillCovers) {
            delete best[item.id];
            improved = true;
      ...
      // ── Schritt 3: Reduziere Übermengen ─────────────────────────────
      // Für jedes Item: wie viele kann man weglassen ohne Anforderungen zu verletzen?
      for (const item of relevant) {
        if (!(best[item.id] > 0)) continue;
        let cur = best[item.id];
        while (cur > 1) {
          const test = { ...best, [item.id]: cur - 1 };
          const ok   = needAttrs.every(a => {
            const tot = relevant.reduce((s, it) => s + (it[a]||0) * (test[it.id]||0), 0);
            return tot >= (needs[a]||0);
          });
          if (ok) { cur--; best[item.id] = cur; }
          else break;
        }
      }

      // Nullen entfernen
      Object.keys(best).forEach(k => { if (!best[k]) delete best[k]; });
      return best;
    };
    ...
    const makeRow = (item, attrs) => {
      ...
      const contribs = relevant.map(a => {
        ...
        const overTxt = isOver ? ` <span class="over">(${t('hc.stat_need')}: ${need})</span>` : '';
    ...
    return `
      <div class="hc-detail-header">
        <button class="hc-detail-back" id="detail-back">${t('hc.back')}</button>
        <div>
        ...
        ${can ? `<div style="font-size:11px;color:#00d4aa;margin-left:8px;white-space:nowrap;">${t('hc.growable')}</div>` : ''}
      </div>
  ...
  _buildSuggestion(h, missingAttrs) {
    ...
    return `
      <div class="sug-title">💡 ${t('hc.missing')}: ${missingAttrs.map(a=>`${a.replace('_',' ')} ${(h[a]||0)-(this._totals()[a]||0)} ${t('hc.more')}`).join(' · ')}</div>
