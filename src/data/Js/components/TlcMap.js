...
class TlcMap extends HTMLElement {
      ...
      <div id="tlc-settings" class="settings-win" style="display:none">
        ...
        <div class="sw-body">
          ...
          <div class="sw-btn-row">
            <button id="sw-export" class="sw-btn sw-export"></button>
            <button id="sw-import" class="sw-btn sw-import"></button>
            <button id="sw-clear"  class="sw-btn sw-clear" ></button>
          </div>
          <input type="file" id="sw-import-file" accept=".json" style="display:none">

          <div class="sw-group-label" id="sw-lbl-lang">Language</div>
          <select id="sw-lang" style="width:100%;background:rgba(0,0,0,0.4);border:1px solid #1a4a5a;color:#b8e4f0;font-family:inherit;font-size:11px;padding:5px 8px;border-radius:4px;margin-bottom:10px;">
          </select>

          <div class="sw-group-label">Visibility</div>
          <label class="sw-check">
            <input type="checkbox" id="chk-hidden">
            <span id="lbl-hidden"></span>
          </label>
          <label class="sw-check">
            <input type="checkbox" id="chk-seedvault">
            <span id="lbl-seedvault"></span>
          </label>
          <label class="sw-check">
            <input type="checkbox" id="chk-caves">
            <span id="lbl-caves"></span>
          </label>
          <label class="sw-check">
            <input type="checkbox" id="chk-labels" checked>
            <span id="lbl-labels"></span>
          </label>
          <label class="sw-check">
            <input type="checkbox" id="chk-fogOfWar" checked>
            <span id="lbl-fogOfWar"></span>
          </label>
  ...
  _renderWaypoint(w, idx, canvas) {
    ...
    `;
    el.title = t('modal.wp_click_remove', { label: w.label || 'WP ' + (idx + 1) });
    el.addEventListener('click', (e) => {
      e.stopPropagation();
  ...
  _updateSettingsLabels() {
    ...
    const pairs = [
      ['#sw-lbl-lang',       'settings.language'],
      ['.sw-title',          'settings.title'],
      ['#sw-export',         'settings.export'],
      ['#sw-import',         'settings.import_btn'],
      ['#sw-clear',          'settings.clear_data_btn'],
      ['#lbl-hidden',        'settings.hidden_loc'],
      ['#lbl-seedvault',     'settings.seed_reserves'],
      ['#lbl-caves',         'settings.caves'],
      ['#lbl-labels',        'settings.place_names'],
      ['#lbl-fogOfWar',      'settings.fog_unknown'],
    ];
  ...
  _initSettings() {
    // Language selector
    const langSel = this.querySelector('#sw-lang');
    if (langSel && !langSel._swBound) {
      langSel._swBound = true;
      langSel.innerHTML = TLC_I18N.getLangs()
        .map(code => `<option value="${code}" ${code === TLC_I18N.getLang() ? 'selected' : ''}>${TLC_I18N.getLangName(code)}</option>`)
        .join('');
      langSel.addEventListener('change', () => {
        TLC_I18N.setLang(langSel.value);
      });
    } else if (langSel) {
      // Update selected option to reflect current lang
      langSel.value = TLC_I18N.getLang();
    }
    this._updateSettingsLabels();
    if (!this._langLabelBound) {
      this._langLabelBound = true;
      window.addEventListener('tlc:langchange', () => this._updateSettingsLabels());
    }
    const vis = window.TLC_STATE.get('visibility') || {};
    ...
    if (clearBtn && !clearBtn._swBound) {
      ...
      clearBtn.addEventListener('click', () => {
        ...
        window.TLC_STATE.save();
        window.tlcNotify(t('notify.data_deleted'), 'warn');
      });
    }
  }
  ...
  _importData(jsonStr) {
    let data;
    try { data = JSON.parse(jsonStr); } catch { window.tlcNotify(t('notify.invalid_file'), 'error'); return; }
    if (!data.version) { window.tlcNotify(t('notify.unknown_format'), 'error'); return; }
    const s = window.TLC_STATE;
  ...
  _openNodeCtx(e, loc) {
    ...
    window.TLC_CTX_MENU.show(e.clientX, e.clientY, [
      ...
      { type: 'sep' },
      { icon: visited2 ? '🙈' : '✅', label: visited2 ? t('ctx.mark_unvisited') : t('ctx.mark_visited'),
        action: () => this._toggleVisited(loc) },
      { icon: 'ℹ️', label: t('ctx.open_info'),  action: () => this._openNodeInfoModal(loc) },
      { icon: '📌', label: t('ctx.set_waypoint'),       action: () => this._openWaypointModal(loc.gx, loc.gy, loc.name) },
    ]);
  }
  ...
  _openMapCtx(e) {
    ...
    window.TLC_CTX_MENU.show(e.clientX, e.clientY, [
      ...
      { type: 'sep' },
      { icon: '📍', label: t('ctx.add_marker'),     action: () => this._openUserMarkerModal(gx, gy) },
      { icon: '⚑',  label: t('ctx.set_waypoint'),            action: () => this._openWaypointModal(gx, gy) },
      { icon: '⚓', label: t('ctx.set_boat'),  action: () => {
  ...
  _openUserMarkerCtx(e, idx) {
    ...
    if (!m) return;
    window.TLC_CTX_MENU.show(e.clientX, e.clientY, [
      { type: 'title', text: m.note || t('ctx.own_marker') },
      { type: 'sep' },
      { icon: '✏️', label: t('ctx.edit_marker'), action: () => this._editUserMarkerModal(idx) },
      { icon: '🗑️', label: t('ctx.delete_marker'),    action: () => {
          const markers = window.TLC_STATE.get('userMarkers');
          markers.splice(idx, 1);
          window.TLC_STATE.set('userMarkers', markers);
          window.TLC_STORAGE.save();
          tlcNotify(t('notify.marker_removed'));
        }},
    ]);
  }
  ...
  _openNodeInfoModal(loc, focusColor = false) {
    ...
    const RES_TYPES = [
      { key: 'battery', label: t('tank.battery'),   unit: 'kW',  cap: 6000,  icon: '🔋' },
      { key: 'oil',     label: t('tank.oil'),          unit: 'L',   cap: 2000,  icon: '🛢️' },
      { key: 'diesel',  label: t('tank.diesel'),      unit: 'L',   cap: 2000,  icon: '⛽' },
      { key: 'petrol',  label: t('tank.petrol'),      unit: 'L',   cap: 2000,  icon: '🔴' },
      { key: 'oxygen',  label: t('tank.oxygen'),  unit: 'L',   cap: 9000, icon: '💨' },
      { key: 'methane', label: t('tank.methane'),      unit: 'L',   cap: 6000,  icon: '🔥' },
      { key: 'co2',     label: t('tank.co2'), unit: 'L',   cap: 6000,  icon: '🌫️' },
      { key: 'water',   label: t('tank.water'),   unit: 'L',   cap: 2000,  icon: '💧' },
    ];
      ...
      : notes.map((n, i) => `
          <div class="note-item">
            ...
            <div class="note-meta">
              ...
              <button class="note-del" data-idx="${i}" title=t('modal.delete')>✕</button>
            </div>
          </div>`).join('');

    // ── Ressourcen-Rechner HTML ───────────────────────────────────────
    /* Enemies aus loc.enemies — gruppiert nach Kategorie
     * Format: { category:t('enemy.large'), entries:[{name:'Roller Boi', count:2}] }
     * oder flach: { name:'Sweetheart', count:1, category:t('enemy.small') }
     */
    const ENEMY_CATEGORIES = [t('enemy.hazards'), t('enemy.small'), t('enemy.large')];
    const enemiesHTML = () => {
      ...
      const cats = {};
      raw.forEach((e, i) => {
        const cat = e.category || t('enemy.other');
    ...
    const resHTML = () => (nd.resources || []).map((r, i) => `
      <div class="res-row">
        ...
        <button class="res-del" data-idx="${i}" title=t('modal.delete')>✕</button>
      </div>`).join('') || `<div class="empty-hint">No Structures</div>`;

    window.TLC_MODAL.open({
      title: t('modal.location_info'),
      body: `
        ${extraCSS}
        ...
        <div class="tab-panel" id="tab-notes">
          ...
          <div class="note-add-row">
            <textarea id="note-input" placeholder="${t('modal.add_note')}" rows="3"></textarea>
      ...
      onOpen: (sr) => {
        ...
        sr.getElementById('res-add').addEventListener('click', () => {
          ...
          if (!label) { tlcNotify(t('modal.enter_label'), 'warn'); return; }
  ...
  _openWaypointModal(gx, gy, defaultName = '') {
    window.TLC_MODAL.open({
      title: t('modal.set_waypoint'),
      body: `
        <div class="field">
          <label>NAME</label>
          <input id="wp-label" type="text" placeholder="${t('modal.wp_name')}" value="${defaultName}">
        </div>
  ...
  _openUserMarkerModal(gx, gy, existing = null) {
    ...
    window.TLC_MODAL.open({
      title: t(isEdit ? 'modal.edit_marker' : 'modal.add_marker'),
      body: `
        <div class="field">
          <label>NOTIZ (optional)</label>
          <input id="um-note" type="text" placeholder="${t('modal.marker_desc')}" value="${isEdit ? (existing.note||'') : ''}">
        </div>
        <div class="field">
          <label>FARBE</label>
      ...
      footer: `
        ...
        <button class="btn-ghost"   id="btn-cancel">Cancel</button>
        <button class="btn-primary" id="btn-save">${t(isEdit ? 'modal.save' : 'modal.set')}</button>
      `,
      onOpen: (sr) => {
        this._bindSwatches(sr);
