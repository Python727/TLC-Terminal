...
const _defaultState = {
  ...
  itemIndexEdits:  {},   // Item Index user edits (foundAt, weight, notes)
};
...
class TlcState {
  ...
  async load(slot = this._activeSlot || 0) {
    ...
    try {
      ...
      ['waypoints','userMarkers','nodeData','boatPos','visibility','activeZone',
       'humanCalcInv','humanCalcStock','humanCalcStatus','itemIndexEdits'].forEach(k => {
        this._data[k] = def[k];
      });
      if (saved) {
        const keys = ['waypoints','userMarkers','nodeData','boatPos','visibility','activeZone',
                      'humanCalcInv','humanCalcStock','humanCalcStatus','itemIndexEdits'];
        keys.forEach(k => {
  ...
  async save() {
    const payload = {
      ...
      itemIndexEdits:  this._data.itemIndexEdits,
    };
    try {
      if (window.TLC_SERVER) {
